...
int main(int argc,char *argv[])
{
  if (argc<2) {
    ...
    char *filename=argv[1], *root=((argc<3)?(NULL):(argv[2])), *cwd=NULL;
	...
	if (cwd) free(cwd); free(content); fclose(f);
	exit(1);}
      ptr=ptr+delta; bytes_read=bytes_read+delta;}
    SHA1Init(&context);
    SHA1Update(&context,content,file_len);
    SHA1Final(digest,&context);
    i=0; while (i<20) {
      int byte=digest[i];
      digest_string[2*i]=hexdigits[byte&0xF];
      digest_string[(2*i)+1]=hexdigits[(byte>>4)&0xF];
      i++;}
    digest_string[i]='\0';
